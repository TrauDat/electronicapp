...
public class BillDetail {
	...
	private int countHouseHole() {
		...
		Optional<HouseHold> houseHoldOpt = houseHoldService.findById(account.getId());
	
		if (houseHoldOpt.isPresent()) {
			listHouseHold = houseHoldOpt.stream().collect(Collectors.toList());
		}
	...
	private double unitMount() {
		...
		Map<Long, ElecPrice> mapElecBill = listElecPrice.stream().collect(Collectors.toMap(ElecPrice::getId, Function.identity()));
	...
	private void init(Bill bill) {
		...
		amountUnit.setText(Double.toString(total));
	}
